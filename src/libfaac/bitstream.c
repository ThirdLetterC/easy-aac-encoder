...
static int CountBitstream(faacEncHandle hEncoder, CoderInfo *coderInfo, ChannelInfo *channelInfo, BitStream *bitStream,
                          int numChannels);
static int WriteADTSHeader(faacEncHandle hEncoder, BitStream *bitStream, int writeFlag);
static int WriteCPE(CoderInfo *coderInfoL, CoderInfo *coderInfoR, ChannelInfo *channelInfo, BitStream *bitStream,
                    int objectType, int writeFlag);
static int WriteSCE(CoderInfo *coderInfo, ChannelInfo *channelInfo, BitStream *bitStream, int objectType,
                    int writeFlag);
static int WriteLFE(CoderInfo *coderInfo, ChannelInfo *channelInfo, BitStream *bitStream, int objectType,
                    int writeFlag);
...
static int WritePredictorData(CoderInfo *coderInfo, BitStream *bitStream, int writeFlag);
...
#ifdef DRM
static int PutBitHcr(BitStream *bitStream, unsigned long curpos, unsigned long data, int numBit);
static int rewind_word(int W, int len);
static int WriteReorderedSpectralData(CoderInfo *coderInfo, BitStream *bitStream, int writeFlag);
...
int WriteBitstream(faacEncHandle hEncoder, CoderInfo *coderInfo, ChannelInfo *channelInfo, BitStream *bitStream,
                   int numChannel)
{
    int channel;
    int bits = 0;
    ...
    for (channel = 0; channel < numChannel; channel++)
    {
        ...
        if (channelInfo[channel].present)
        {
            ...
            if (!channelInfo[channel].cpe)
            {
                ...
                {
                    /* Write out lfe */
                    bits += WriteLFE(&coderInfo[channel], &channelInfo[channel], bitStream,
                                     hEncoder->config.aacObjectType, 1);
                }
                else
                {
                    /* Write out sce */
                    bits += WriteSCE(&coderInfo[channel], &channelInfo[channel], bitStream,
                                     hEncoder->config.aacObjectType, 1);
                }
            }
            else
        ...
        if (channelInfo[channel].present)
        {
            ...
            if (!channelInfo[channel].cpe)
            {
                ...
                if (channelInfo[channel].lfe)
                {
                    /* Write out lfe */
                    bits += WriteLFE(&coderInfo[channel], &channelInfo[channel], bitStream,
                                     hEncoder->config.aacObjectType, 0);
                }
                else
                {
                    /* Write out sce */
...
static int WritePredictorData(CoderInfo *coderInfo, BitStream *bitStream, int writeFlag)
{
    ...
    if (writeFlag)
    {
        if (predictorDataPresent)
        {
            ...
            else
            {
                ...
                PutBit(bitStream, (unsigned long)coderInfo->reset_group_number, LEN_PRED_RSTGRP);
            }
    ...
    bits += (predictorDataPresent)
                ? (LEN_PRED_RST + ((coderInfo->reset_group_number) != -1) * LEN_PRED_RSTGRP + numBands * LEN_PRED_ENAB)
                : 0;
...
static int WriteSpectralData(CoderInfo *coderInfo, BitStream *bitStream, int writeFlag)
{
    ...
    if (writeFlag)
    {
        ...
        {
            if (len[i] > 0)
            {                                       /* only send out non-zero codebook data */
...
static const unsigned short PresortedCodebook_VCB11[] = {11, 31, 30, 29, 28, 27, 26, 25, 24, 23, 22,
                                                         21, 20, 19, 18, 17, 16, 9,  7,  5,  3,  1};
static const int maxCwLen[32] = {0,  11, 9,  20, 16, 13, 11, 14, 12, 17, 14, 49, 0,  0,  0,  0,
                                 14, 17, 21, 21, 25, 25, 29, 29, 29, 29, 33, 33, 33, 37, 37, 41}; /* 8.5.3.3.3.1 */
...
static int WriteReorderedSpectralData(CoderInfo *coderInfo, BitStream *bitStream, int writeFlag)
{
    ...
    if (writeFlag)
    {
        ...
        for (i = 0; i < num_cw; i++)
        {
            ...
            if (coeff_cnt - last_sfb >= cur_sfb_len)
            {
                ...
                if (win_cnt == window_group_length[win_grp_cnt])
                {
                    ...
                    {
                        cur_sfb_len =
                            (sfb_offset[sfb_cnt + 1] - sfb_offset[sfb_cnt]) / window_group_length[win_grp_cnt];
                    }
                }
            }
        }
        ...
        if (0)
        { // coderInfo->block_type == ONLY_SHORT_WINDOW) {
        ...
        for (presort = 0; presort < LEN_PRESORT_CODEBOOK; presort++)
        {
            ...
            for (i = 0; i < num_cw; i++)
            {
                ...
                if ((cw_info_preso[i].cb == nextCB) || ((nextCB < ESC_HCB) && (cw_info_preso[i].cb == nextCB + 1)))
                {
                        ...
                        for (cw_part_cnt = 0; cw_part_cnt < cw_info[codeword_index].num_data; cw_part_cnt++)
                        {
                            ...
                            if (len[cur_cw_part] <= tmplen)
                            {
                                ...
                                if (is_backwards)
                                {
                                    ...
                                    PutBitHcr(bitStream,
                                              startbitpos + segment[segment_index].right - len[cur_cw_part] + 1,
                                ...
                                {
                                    PutBitHcr(bitStream, startbitpos + segment[segment_index].left, data[cur_cw_part],
                                              len[cur_cw_part]);
                            ...
                            else
                            {
                                ...
                                {
                                    PutBitHcr(bitStream, startbitpos + segment[segment_index].left, tmp_data, tmplen);
