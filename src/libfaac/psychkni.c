...
static void PsyCheckShort(PsyInfo *psyInfo)
{
    ...
    psyInfo->block_type = ((totchg > 1.0) && (totchg2 > 0.04)) ? ONLY_SHORT_WINDOW : ONLY_LONG_WINDOW;
...
static void PsyInit(GlobalPsyInfo *gpsyInfo, PsyInfo *psyInfo, unsigned int numChannels, unsigned int sampleRate,
                    int *cb_width_long, int num_cb_long, int *cb_width_short, int num_cb_short)
{
    ...
    for (channel = 0; channel < numChannels; channel++)
    {
        ...
        for (j = 0; j < 8; j++)
        {
            psydata->fftEnrgPrevS[j] = (psyfloat *)AllocMemory(NSFB_SHORT * sizeof(psyfloat));
            memset(psydata->fftEnrgPrevS[j], 0, NSFB_SHORT * sizeof(psyfloat));
            psydata->fftEnrgS[j] = (psyfloat *)AllocMemory(NSFB_SHORT * sizeof(psyfloat));
            memset(psydata->fftEnrgS[j], 0, NSFB_SHORT * sizeof(psyfloat));
            psydata->fftEnrgNextS[j] = (psyfloat *)AllocMemory(NSFB_SHORT * sizeof(psyfloat));
            memset(psydata->fftEnrgNextS[j], 0, NSFB_SHORT * sizeof(psyfloat));
            psydata->fftEnrgNext2S[j] = (psyfloat *)AllocMemory(NSFB_SHORT * sizeof(psyfloat));
...
static void PsyCalculate(ChannelInfo *channelInfo, GlobalPsyInfo *gpsyInfo, PsyInfo *psyInfo, int *cb_width_long,
                         int num_cb_long, int *cb_width_short, int num_cb_short, unsigned int numChannels)
{
    unsigned int channel;
...
static void PsyBufferUpdate(FFT_Tables *fft_tables, GlobalPsyInfo *gpsyInfo, PsyInfo *psyInfo, double *newSamples,
                            unsigned int bandwidth, int *cb_width_short, int num_cb_short)
{
    int win;
