...
static void CalcAllowedDist(CoderInfo *coderInfo, PsyInfo *psyInfo, double *xr, double *xmin, int quality);

void AACQuantizeInit(CoderInfo *coderInfo, unsigned int numChannels, AACQuantCfg *aacquantCfg)
{
...
void AACQuantizeEnd(CoderInfo *coderInfo, unsigned int numChannels, AACQuantCfg *aacquantCfg)
{
    ...
    for (channel = 0; channel < numChannels; channel++)
    {
        if (coderInfo[channel].requantFreq)
            FreeMemory(coderInfo[channel].requantFreq);
    }
}

static void BalanceEnergy(CoderInfo *coderInfo, const double *xr, const int *xi, double *pow43)
{
...
static void UpdateRequant(CoderInfo *coderInfo, int *xi, double *pow43)
{
    ...
    {
        double invQuantFac = pow(2.0, -0.25 * (coderInfo->scale_factor[sb] - coderInfo->global_gain));
...
int AACQuantize(CoderInfo *coderInfo, PsyInfo *psyInfo, ChannelInfo *channelInfo, int *cb_width, int num_cb, double *xr,
                AACQuantCfg *aacquantCfg)
{
    ...
    if (do_q)
    {
        ...
        coderInfo->global_gain = 0;
        FixNoise(coderInfo, xr, xr_pow, xi, xmin, aacquantCfg->pow43, aacquantCfg->adj43);
    ...
    // clamp to valid diff range
    {
        ...
        for (i = 0; i < coderInfo->nr_of_sfb; i++)
        {
            if ((coderInfo->book_vector[i] == INTENSITY_HCB) || (coderInfo->book_vector[i] == INTENSITY_HCB2))
            {
                ...
                if (diff < -60)
                    scale_factor[i] = previous_is_factor - 60;
                else if (diff > 59)
                    scale_factor[i] = previous_is_factor + 59;
            ...
            else if (coderInfo->book_vector[i])
            {
                ...
                if (diff < -60)
                    scale_factor[i] = previous_scale_factor - 60;
                else if (diff > 59)
                    scale_factor[i] = previous_scale_factor + 59;
    ...
    for (i = 0; i < coderInfo->nr_of_sfb; i++)
    {
        OutputBits(coderInfo,
#ifdef DRM
                   &coderInfo->book_vector[i], /* needed for VCB11 */
#else
                   coderInfo->book_vector[i],
#endif
                   xi, coderInfo->sfb_offset[i], coderInfo->sfb_offset[i + 1] - coderInfo->sfb_offset[i]);
...
#endif
static void QuantizeBand(const double *xp, int *pi, double istep, int offset, int end, double *adj43)
{
    int j;
    fi_union *fi;
...
#endif
static void QuantizeBand(const double *xp, int *ix, double istep, int offset, int end, double *adj43)
{
    int j;
...
static void CalcAllowedDist(CoderInfo *coderInfo, PsyInfo *psyInfo, double *xr, double *xmin, int quality)
{
    ...
    for (sfb = 0; sfb < num_cb; sfb++)
    {
        ...
        xmin[sfb] = ((coderInfo->block_type == ONLY_SHORT_WINDOW) ? 0.65 : 1.12) * globalthr * thr;
    }
}
                    ...
                    double *adj43)
{
    ...
    for (sb = 0; sb < coderInfo->nr_of_sfb; sb++)
    {
        ...
        QuantizeBand(xr_pow, xi, IPOW20(coderInfo->global_gain), start, end, adj43);
        // printf("\tsfac: %d\n", sfac);
    ...
    calcdist:
        ...
        if (fabs(fixstep) >= maxstep)
        {
            if ((dist && (fixstep < 0)) || (!dist && (fixstep > 0)))
            {
                fixstep = -0.5 * fixstep;
            }
...
int SortForGrouping(CoderInfo *coderInfo, PsyInfo *psyInfo, ChannelInfo *channelInfo, int *sfb_width_table, double *xr)
{
    int i, j, ii;
    int index = 0;
