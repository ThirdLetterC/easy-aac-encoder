...
void HuffmanEnd(CoderInfo *coderInfo, unsigned int numChannels)
{
    ...
    for (channel = 0; channel < numChannels; channel++)
    {
        if (coderInfo[channel].data)
            FreeMemory(coderInfo[channel].data);
        if (coderInfo[channel].len)
            FreeMemory(coderInfo[channel].len);

#ifdef DRM
        if (coderInfo[channel].num_data_cw)
            FreeMemory(coderInfo[channel].num_data_cw);
#endif
    }
}

int BitSearch(CoderInfo *coderInfo, int *quant) /* Quantized spectral values */
                                                /*
    ...
    i = 0;
        ...
        for (j = pow2levels; j < fracpow2lev; j++)
        {
...
#ifdef SLOW
            if (i > 0)
            { /* not on the lowest level, grouping more than one signle scalefactor band per section*/
                if (bit_stats[j][0] < bit_stats[2 * j][0] + bit_stats[2 * j + 1][0])
                {
                    ...
                    for (n = k; n < k + hop; n++)
                    { /* write the optimal huffman book value for the new larger section */
                        if ((book_vector[n] != INTENSITY_HCB) && (book_vector[n] != INTENSITY_HCB2))
                        { /* Don't merge with IS bands */
                            book_vector[n] = bit_stats[j][1];
                        }
                    }
                }
                else
                { /* it was cheaper to transmit the smaller huffman table sections */
            ...
            { /* during the first stage of the iteration, all sfb's are individual sections */
                ...
                {
                    book_vector[k] =
                        bit_stats[j][1]; /* initially, set all sfb's to their own optimal section table values */
                }
            }
...
int NoiselessBitCount(CoderInfo *coderInfo, int *quant, int hop, int min_book_choice[112][3])
{
    int i, j, k;
    ...
    for (i = 0; i < nr_of_sfb; i = i + hop)
    {
...

        {
             ...
             */
            max_sb_coeff = 0;
            for (j = sfb_offset[i]; j < sfb_offset[q]; j++)
            { /* snl */
            ...
            { /* if the section does have non-zero coefficients */
                if (max_sb_coeff < 2)
                {
...
int OutputBits(CoderInfo *coderInfo,
               ...
               int book,
#endif
               int *quant, int offset, int length)
{
    /*
       This function inputs
    ...
    case 3:
        for (i = offset; i < offset + length; i = i + 4)
        {
            ...
            {
                if (quant[i + j] > 0)
                { /* send out '0' if a positive value */
                    data[counter] = 0;
                    len[counter++] = 1;
                    bits += 1;
#ifdef DRM
                ...
                { /* send out '1' if a negative value */
                    data[counter] = 1;
                    len[counter++] = 1;
                    bits += 1;
#ifdef DRM
    ...
    case 11:
        ...
        for (i = offset; i < offset + length; i = i + 2)
        {
            if ((ABS(quant[i]) >= 16) && (ABS(quant[i + 1]) >= 16))
            { /* both codewords were above 16 */
                /* first, code the orignal pair, with the larger value saturated to +/- 16 */
                index = 17 * 16 + 16;
            }
            else if (ABS(quant[i]) >= 16)
            { /* the first codeword was above 16, not the second one */
                /* first, code the orignal pair, with the larger value saturated to +/- 16 */
                index = 17 * 16 + ABS(quant[i + 1]);
            }
            else if (ABS(quant[i + 1]) >= 16)
            { /* the second codeword was above 16, not the first one */
                index = 17 * ABS(quant[i]) + 16;
            }
            else
            { /* there were no values above 16, so no escape sequences */
...
int CalcBits(CoderInfo *coderInfo, int book, int *quant, int offset, int length)
{
    /*
       This function inputs
       - a specific codebook number, 'book'
...
int SortBookNumbers(CoderInfo *coderInfo, BitStream *bitStream, int writeFlag)
{
    ...
    { /* the block_type is a long,start, or stop window */
        max = 31;
        bit_len = 5;
    }
    ...
    for (g = 0; g < coderInfo->num_window_groups; g++)
    {
        ...
        for (i = band + 1; i < band + max_sfb; i++)
        {
                ...
                if ((book_vector[i] != previous))
            {
                ...
                { /* in case you need to terminate an escape sequence */
                    if (writeFlag)
...
#endif
        {
            ...
            if (repeat_counter == max)
            { /* special case if the last section length is an */
                /* escape sequence */
                if (writeFlag)
                    PutBit(bitStream, 0, bit_len);
                bit_count += bit_len;
...
int WriteScalefactors(CoderInfo *coderInfo, BitStream *bitStream, int writeFlag)

{
    ...
    for (j = 0; j < coderInfo->num_window_groups; j++)
    {
        for (i = 0; i < nr_of_sfb_per_group; i++)
        {
            ...
            if ((coderInfo->book_vector[index] == INTENSITY_HCB) || (coderInfo->book_vector[index] == INTENSITY_HCB2))
            {
